...
Квиз живет на `/<locale>/brief`, страница «спасибо» — на `/<locale>/brief/thanks`. Верстка одна на
пять направлений (`src/components/Brief/`): она построена только на токенах, поэтому у каждого стиля
своя палитра, шрифты и радиусы без отдельных реализаций. Артборда у квиза нет — на `design/*.dc.html`
нарисована только главная. Миниатюры стилей на третьем шаге рисуются токенами чужого направления:
блоки `[data-design='<name>']` лежат в общем CSS, поэтому атрибуты `data-design` и `data-theme` прямо
на узле дают его палитру внутри текущего стиля. Настоящие шрифты направления в миниатюру не приезжают
(они в его CSS-чанке), поэтому миниатюра абстрактная, без текста.

Ответы шагов — ключи справочников `BRIEF_STEP_VALUES` (`src/lib/brief/consts.ts`), а не переведенный
текст: значение вне справочника на сервер не проходит. Продукты первого шага берутся из `PRICING_PLANS`,
поэтому новый тариф появляется в квизе сам, а `?plan=` с кнопки прайса предвыбирает первый шаг.

Заявку принимает `POST /api/brief` (`src/app/api/brief/route.ts`): лимит по адресу, разбор через
`parseLead`, затем `deliverLead` в три канала параллельно — Telegram (по одному запросу на каждый id
из `TELEGRAM_CHAT_IDS`), письмо через SMTP и лид в Bitrix24. Упавший канал не уносит остальные: ошибки
собираются и пишутся в лог, 502 отдается только когда молчат все три. Тексту заявки (`formatLead`)
нужна нижняя часть с `ключ=значение` — по ней тесты и будущие отчеты находят источник, нишу, стиль
и метки; менять формат, не поправив `tests/unit/brief.test.ts`, нельзя.

Метки кампании снимает `src/proxy.ts` и кладет в cookie `marks`: по рекламе приходят на главную,
а заявку оставляют на брифе, и к моменту отправки меток в адресе уже нет.

Ловушка для ботов — поле `zone` (`HONEYPOT_FIELD`), спрятанное за экран. Имя нарочно вне словаря
автозаполнения: на `company` браузер подставил бы организацию живому человеку, и его заявка молча
пропала бы. Заполнена — ответ 200 без единой отправки: по ответу бот не должен понимать, что его
отбили. Без чекбокса согласия кнопка не нажимается, а прямой запрос получает 422.

Якоря в шапке абсолютные — `/<locale>#<секция>`, а не `#<секция>`: шапка рендерится на каждой
странице, а секции с этими `id` лежат только на главной, и относительный якорь мертв везде, кроме
нее. `id` секций: `services` и `prices` в `Pricing.tsx`, `process` в `Process.tsx`, `contacts`
в `Footer.tsx`. Проверку держит `tests/e2e/links.spec.ts` — он ищет якорь локатором в DOM той
...
Заявка в e2e уходит в настоящие каналы, только приемником служит `tests/e2e/lead-sink.ts`: HTTP-сервер
для Telegram и Bitrix24 и настоящий SMTP-сервер для письма, оба поднимаются третьим `webServer`
в конфиге Playwright, а адреса каналов подменяются через `LEAD_ENV`. Письмо приходит закодированным,
поэтому приемник раскодирует тело (base64 и quoted-printable) — иначе вместо `source=site` в нем
лежало бы `source=3Dsite`. Тесты отбирают свои записи по маркеру в имени заявки, поэтому параллельные
прогоны друг другу не мешают. Адреса каналов приезжают только из `LEAD_ENV`, поэтому сервер, поднятый
руками (`yarn start -p 3100`), для этих тестов не годится: `reuseExistingServer` его подхватит, и заявка
получит 502. Перед полным прогоном сервера на 3100, 3101 и 3103 надо гасить. Лимит заявок живет в памяти
сервера и переживает прогоны, поэтому спек квиза шлет свой `x-forwarded-for` на каждый запуск.
